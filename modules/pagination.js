...
const paginate = async (req, limit, getAndCountAll) => {
  ...
  const { count, rows } = await getAndCountAll(offset, limit, req.query);

  if (rows.length === 0 && offset !== 0)
    throw errorMessage(req.query.page, "the page does not exist");
